...
% Graph
\begin{scope}[shift={($(hex.center)+(-.2,-0.9)$)}]
\begin{scope}[xscale=0.95]
  ...
  \draw[Larrow] (L2c) to[out=30,in=150,looseness=0.88] (Ltc);
  \draw[Larrow] (L1c) to[out=52,in=128,looseness=0.88] (Ltc);

  % Highest lane: L-history arrows to Y
  \draw[Larrow] (L1c) to[out=66,in=128,looseness=0.92] (Yc);
  \draw[Larrow] (L2c) to[out=55,in=138,looseness=0.86] (Yc);
  \draw[Larrow] (Ltc) to[out=25,in=150,looseness=0.82] (Yc);
